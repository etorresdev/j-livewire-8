...
@section('content')
    <div class="grid grid-cols-3 gap-4">
        <div class="p-8 bg-gray-200 col-span-1">
            <ul class="flex flex-col">
                <li class="font-medium text-sm text-gray-600 uppercase mb-4">Contenido</li>
                @foreach ($course->posts as $post)
                <li class="flex items-center text-gray-600 my-2">
                    {{ $post->name }}
                    @if ($post->free)
                        <span class="text-xs text-gray-500 font-semibold bg-gray-400 px-2 rounded-full ml-auto">Gratis</span>
                    @endif
                </li>
                <br>
                @endforeach
    ...
    <div class="text-center">
        ...
        <h2 class="text-xl text-gray-600">Fórmate online cómo profesional</h2>
    </div>
